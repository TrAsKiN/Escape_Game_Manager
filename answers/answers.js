...
exports.run = (message, main) => {
  ...
  let Serv       = main.guilds.find(s => s.name === (playerData[message.author.id][playerData[message.author.id].length - 1]))
   if (Serv === null) {return message.reply('Vous n\'êtes pas enregistré dans ce serveur. Tapez la commande de départ pour vous enregistrer')}
	...
	catch (err){message.channel.send('Erreur de chargement des fichiers. Parlez en au MJ')}
