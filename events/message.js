...
main.on('message', message => {
  ...
  if (message.content === '!edit') { permchk(edit(message)) }
  if (message.content === '!commandes') {permchk ( message.channel.send( {embed: require('../edition/commandslist.js')} ) )}
  if (message.content.toLowerCase().startsWith("#")) {permchk( require(`../edition/command.js`)(message) )}
    ...
    if (data.answers[`etape 0`].file !== {}) {
      ...
      return message.author.createDM().then(channel => channel.send(suc, att))
		...
		else {return message.author.createDM().then(channel => channel.send(suc))}

           fs.writeFile('./players.json',
                   JSON.stringify(pFile, null, 2),
