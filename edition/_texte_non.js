...
module.exports = (message, namel) => {
  ...
  let data     = JSON.parse(fs.readFileSync(`./guilds/${message.channel.guild.name}/${message.channel.guild.name}.json`))
    ...
    if (namel === 'etape 0') return message.channel.send(`Ceci est l'étape de départ, elle ne peut pas avoir de texte d'échec`)
             ...
             ("module.exports = " + JSON.stringify(arr)),
              function(err) {
               if(err) {
							  ...
							  fs.writeFile((filePath + pos +'.js'), ("module.exports = " + JSON.stringify(arr)),
                 function(err) { if(err) {return message.channel.send('Error') }}
                )
               }
              }
            )
