...
module.exports = (message, namel) => {
	  ...
	  if (!data.answers.hasOwnProperty(namel)) {message.channel.send('L\'étape en question ne semble pas exister')}
	  else {
			message.channel.send(`Entrez le code pour l'${namel} précédé de #.\nPour ajouter plusieurs possibilités de codes, entrez à la suite en les séparant par "#" sans espace. exemple: * #code1#code2#code3*`)
